...
const BLOB_KEY = 'pricing-settings';
...
exports.handler = async function (event) {
  ...
  if (event.httpMethod === 'GET') {
    try {
      ...
      if (!text) return jsonResponse(404, { error: 'No settings saved yet' });
      return jsonResponse(200, JSON.parse(text));
    } catch (err) {
      console.error('[manage-settings] GET error:', err.message);
      return jsonResponse(500, { error: 'Failed to load settings: ' + err.message });
    }
  }
  ...
  if (event.httpMethod === 'PUT') {
    ...
    try {
      ...
      const bs = blobStore();
      await bs.set(BLOB_KEY, JSON.stringify(settings));
      return jsonResponse(200, { ok: true });
    } catch (err) {
      console.error('[manage-settings] PUT error:', err.message);
      return jsonResponse(500, { error: 'Failed to save settings: ' + err.message });
    }
  }
